...
// of "가이드 분석" is fixed by which endpoint was called, not by a
// client-sent operation label — mirrors /api/ai/suggest-order exactly.
const OPERATION = "GUIDE_ANALYZE" as const;
const CREDITS_NEEDED = OPERATION_CREDIT_COST.GUIDE_ANALYZE;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error(`[/api/ai/analyze-guide] 실패 (${errorType}):`, error);
    await logAiUsage(supabase, {
      userId: user.id,
      collaborationId,
      feature: "TEXT_GENERATION",
