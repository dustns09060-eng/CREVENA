...
// Dedicated route (instead of sharing /api/ai/generate) so the
// OPERATION_CREDIT_COST.REELS_PLAN cost of "릴스 장면 구성" is fixed by
// which endpoint was called, not by a client-sent operation label — a
// client can't relabel this as a 1-credit call to pay less. Otherwise
// mirrors /api/ai/generate exactly, including the maxTokens passthrough
// the (large) scene-composition prompt needs.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error(`[/api/ai/reels-plan] 실패 (${errorType}):`, error);
    await logAiUsage(supabase, {
      userId: user.id,
      collaborationId,
      feature: "TEXT_GENERATION",
