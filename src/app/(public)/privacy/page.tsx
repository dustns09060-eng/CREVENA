  ...
  return (
    ...
    >
      ...
      <LegalSection heading="1. 수집하는 개인정보 항목">
        <LegalText>
          서비스는 회원가입 시 <b>이메일 주소와 비밀번호만</b> 수집합니다. 이름, 생년월일, 성별, 휴대전화번호는 수집하지
          않으며, 소셜 로그인(카카오·네이버·구글 등)은 사용하지 않습니다. 그 밖의 정보는 회원이 서비스를 이용하는 과정에서
          직접 입력하거나 자동으로 생성됩니다.
        </LegalText>
        <LegalTable
          headers={["구분", "항목", "수집 시점"]}
          rows={[
            ["계정 정보", "이메일 주소, 비밀번호(암호화되어 저장), 표시 이름(선택)", "회원가입 시"],
            [
              "협찬 정보",
        ...
        <LegalText>
          ...
          입력되어 결제대행사가 처리하며, 서비스는 정기결제에 필요한 <b>빌링키</b>만 저장합니다.
        </LegalText>
        <LegalText>
      ...
      <LegalSection heading="2. 개인정보의 이용 목적">
        <LegalList
          items={[
            ...
            "서비스 관련 필수 안내(이메일 인증, 결제 실패 안내 등) 발송",
      ...
      <LegalSection heading="3. 개인정보 처리의 위탁 및 국외 이전">
        ...
        />
        <LegalText>
          <b>Anthropic에 전달되는 정보</b>: AI 기능 실행 시 회원이 입력한 가이드·후기 텍스트와 분석 대상 <b>사진 원본
          이미지</b>가 전달됩니다. 영상의 경우 <b>원본 영상은 전달되지 않고</b>, 회원의 브라우저에서 추출한 최대 3장의
          대표 프레임 이미지만 전달됩니다.
        </LegalText>
        <LegalText>
      ...
      <LegalSection heading="4. 개인정보의 보유 및 파기">
        <LegalText>
          ...
          회원 탈퇴 시에는 계정에 연결된 협찬 정보, 생성 콘텐츠, 업로드 파일이 함께 삭제됩니다.
        </LegalText>
        <LegalText>
          다만 관련 법령에 따라 일정 기간 보존해야 하는 거래·결제 기록은 해당 기간 동안 분리하여 보관한 후 파기합니다.
        </LegalText>
        <OperatorTodo>
          <b>[운영자 확인 필요]</b> 현재 코드에는 <b>보유 기간을 정하는 로직도, 회원이 스스로 탈퇴를 완료하는 기능도
          구현되어 있지 않습니다.</b> 정식 오픈 전에 (1) 항목별 보유 기간(특히 법정 보존이 필요한 결제 기록의 보존 기간),
          (2) 탈퇴 처리 절차와 담당 채널, (3) 탈퇴 시 저장소에 남는 파일까지 삭제하는 절차를 확정하여 이 항목에 기재해야
          합니다. 사실과 다른 보유 기간을 임의로 적지 않았습니다.
        </OperatorTodo>
      </LegalSection>
      ...
      <LegalSection heading="5. 이용자의 권리와 행사 방법">
        <LegalList
          items={[
            ...
            "회원 탈퇴를 원하는 경우에도 아래 문의 채널로 요청하실 수 있습니다.",
          ]}
        />
        <LegalText>
          권리 행사 요청은{" "}
      ...
      <LegalSection heading="6. 개인정보의 안전성 확보 조치">
        <LegalList
          items={[
            ...
            "사진·영상 조회 시에는 유효 시간이 제한된 일회성 링크가 발급됩니다.",
      ...
      <LegalSection heading="7. 만 14세 미만 아동의 개인정보">
        <LegalText>
          서비스는 만 14세 미만 아동을 대상으로 하지 않으며, 만 14세 미만 아동의 회원가입을 받지 않습니다. 만 14세 미만
          아동의 개인정보가 수집된 사실을 알게 된 경우 지체 없이 해당 정보를 파기합니다.
        </LegalText>
      </LegalSection>

      <LegalSection heading="8. 개인정보 보호책임자 및 문의">
        <LegalText>
          개인정보 처리에 관한 문의, 불만 처리, 피해 구제에 관한 사항은 아래 문의 채널로 접수하실 수 있습니다.
        </LegalText>
        <OperatorTodo>
          <b>[운영자 확인 필요]</b> <b>개인정보 보호책임자의 성명·직책·연락처(이메일)</b>와 사업자 정보는 사업자 등록 및
          담당자 지정 후 기재해야 합니다. 저장소 내에 확정된 값이 없어 임의의 이름·주소·이메일을 기재하지 않았습니다.
        </OperatorTodo>
      </LegalSection>
